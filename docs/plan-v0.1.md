...
| 4 | **The product has no way to say *unsaved*, and needs one** — `detached` is terminal and invisible, and P-2, a Must, is false in it | **Reframed after the conflict banner was looked at for the first time**, which changed what fixing this means. There is no dirty indicator in the tab strip and never was — the `●` is the *conflict* marker. Under D-5 that is correct: autosave means there is normally nothing to indicate. But in the **two** states where autosave is suspended and the buffer stays dirty indefinitely — detached, and conflicted — nothing says so. **The state whose entire hazard is "your edits are not being saved" has no indicator for that.** So the fix is not "add a banner for detached"; it is one affordance meaning *unsaved*, shown in exactly the two states where it is true and nowhere else, which keeps D-5's promise intact for the normal case. The detached glyph also names the **cause** (*"removed on disk"*) where the user needs the **consequence** (*"your edits are no longer being saved anywhere"*), and both markers are `aria-hidden`, so a screen reader gets neither. The only exit from a detached tab is closing it, which P-2 guarantees is always safe, and which takes the buffer with it. So this is not a defect with a workaround: **the workaround is the violation.** Read as "a tab gets stuck and you close it" it is an annoyance with an escape; read correctly it is a requirement failure with no escape that does not lose data. The priority follows from that rather than from how the symptom reads. |
